...
import { Product, type ProductCategory } from "@/components/ProductCard";
...
export default async function Home() {
  ...
  return (
    <div className="h-1 grow flex flex-col items-center gap-4">
      ...
      <div>
        <Link href={"/product"} className="px-4 py-2 bg-blue-500 text-white rounded">
          View all Products
        </Link>
      </div>
      ...
      <div className="flex flex-wrap gap-4 justify-center px-8">
        {categories.map((category) => (
          <Link key={category.slug} href={`/category/${category.slug}`} className="px-4 py-2 bg-zinc-200 text-gray-800 rounded-xl shadow-xl">
            {category.name}
          </Link>
        ))}
      </div>
